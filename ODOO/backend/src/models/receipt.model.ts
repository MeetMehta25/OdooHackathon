...
export class ReceiptModel {
  static async create(data: {
    ...
    await pool.query(
      ...
      [id, data.supplier_name, reference_no, 'draft', data.warehouse_id, data.created_by]
    );
  ...
  static async findAll(filters: {
    ...
    let query = 'SELECT r.*, u.full_name as created_by_name, w.name as warehouse_name FROM receipts r JOIN users u ON r.created_by = u.id JOIN warehouses w ON r.warehouse_id = w.id WHERE 1=1';
    const params: any[] = [];
  ...
  static async updateStatus(id: string, status: string): Promise<void> {
    await pool.query('UPDATE receipts SET status = ? WHERE id = ?', [status, id]);
  }
  ...
  static async addItem(data: ReceiptItem): Promise<string> {
    ...
    await pool.query(
      ...
      [id, data.receipt_id, data.product_id, data.quantity_expected, 0, data.location_id]
    );
    return id;
  }
  ...
  static async updateItemReceived(itemId: string, quantity: number): Promise<void> {
    await pool.query('UPDATE receipt_items SET quantity_received = ? WHERE id = ?', [quantity, itemId]);
  }
}