...
/* sample card data (used when MagicBento renders its own grid) */
...
/* ParticleCard: can wrap children (used for wrapping your StatCard or any element) */
...
/* Global spotlight element (keeps original behavior) */
...
/* BentoCardGrid layout used when MagicBento renders its own cards */
...
/* ---------- MagicBento component now accepts children ---------- */
/* If children are provided, MagicBento will wrap the children in a ParticleCard
   so your own element receives tilt/particles/magnetism/click ripple. If no children,
   the original grid of animated cards is rendered. */

const MagicBento: React.FC<React.PropsWithChildren<BentoProps>> = ({
  children,
  textAutoHide = true,
  enableStars = false,
  ...
  /* If the caller passed children, wrap them in ParticleCard so they get the effects */
  if (children) {
    return (
      <ParticleCard
        className={`card flex relative w-full h-full p-0 ${enableBorderGlow ? "card--border-glow" : ""}`}
        ...
        style={{
          // minimal defaults so your wrapped element keeps its sizing
  ...
  /* Otherwise, render the original MagicBento grid */
  return (
    <>
      <style>
        {`
